...
class AdminController extends Controller
{
    ...
    public function addStudent(Request $request)
    {
        ...
        // $adminMaapepicUrl = 'https://admin.maapepic.progfeel.co.in/dashboard/Api/check_username_already_exist_in_adminmaapepic';
        $adminMaapepicUrl = 'https://admin.maapepic.com/dashboard/Api/check_username_already_exist_in_adminmaapepic';
        ...
        // if(isset($ciResponse['status']) && $ciResponse['status'] == 'success') 
        // {
            ...
            */
            ...
            if($request->reference == "2")
            {
                ...
                // $adminMaapepicUrl1 = 'https://admin.maapepic.progfeel.co.in/dashboard/Api/add_external_student';
                $adminMaapepicUrl1 = 'https://admin.maapepic.com/dashboard/Api/add_external_student';
    ...
    public function importStudents(Request $request)
    {
        ...
        foreach ($rows[0] as $row) {
            ...
            $dob11 = trim($row['dob']); // from Excel

            if (!is_numeric($dob11)) {
                // If it's text like "12-10-1995" or "12/10/1995"
                $dob11 = str_replace('-', '/', $dob11); // normalize to d/m/Y
                
            
              
                $dateObj = \DateTime::createFromFormat('d/m/Y', $dob11);
                if ($dateObj) {
                    // Convert DateTime to Excel numeric serial
                    $dob11 = Date::PHPToExcel($dateObj); 
                }
            }
            
            // Now $dob11 is always numeric like 34984
            $dateTimeObject = Date::excelToDateTimeObject($dob11);
            // echo $dateTimeObject->format('Y-m-d');die;
            // echo "Numeric value: " . $dob11 . "\n";
            // echo "Formatted date: " . $dateTimeObject->format('Y-m-d');
  
            // $dateTimeObject = \PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject($dob11);
            // $obj->DOB = $dateTimeObject;
            $obj->DOB = $dateTimeObject->format('Y-m-d');
            $obj->standard = $row['standard'];
            $obj->age = $row['age'];
            $obj->address = $row['address'];
            if(isset($row['paid_user']) && $row['paid_user'] == 'yes') {
